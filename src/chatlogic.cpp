...
void ChatLogic::LoadAnswerGraphFromFile(std::string filename)
{
            ...
            if (type != tokens.end())
            {
                ...
                if (idToken != tokens.end())
                {
                    ...
                    if (type->second == "NODE")
                    {
                        ...
                        //auto newNode = std::find_if(_nodes.get()[0], _nodes.get()[_nodes.use_count()], [&id](GraphNode *node) { return node->GetID() == id; });

                        // create new element if ID does not yet exist
                        if (newNode == _nodes.end())
                        //if (newNode == _nodes.get()[_nodes.use_count()])
                        {
                            //std::unique_ptr<GraphNode> unique(new GraphNode(id));
                            _nodes.emplace_back(std::make_unique<GraphNode>(id));
                            //_nodes.emplace_back(new GraphNode(id));
                            newNode = _nodes.end() - 1; // get iterator to last element

                            //newNode = _nodes.get()[_nodes.use_count()] - 1; // get iterator to last element
    ...
    //ChatBot *local = new ChatBot("../images/chatbot.png");
    ChatBot local("../images/chatbot.png");
    SetChatbotHandle(&local);
